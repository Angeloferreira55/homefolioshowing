...
import { LogIn, Home, Eye, EyeOff, Mail, Apple, Phone, ArrowLeft } from 'lucide-react';
...
// Convert display phone to E.164 format: +1XXXXXXXXXX
...
const Auth = () => {
  ...
  const sendOtp = useCallback(async (phoneNumber: string, userId?: string | null) => {
    try {
  ...
  const handleResendOtp = async () => {
    ...
    setOtpLoading(true);
    const sent = await sendOtp(phone, signupUserId);
    if (sent) {
      startResendTimer();
  ...
  const validateForm = () => {
    try {
      ...
      if (error instanceof z.ZodError) {
        const fieldErrors: { email?: string; password?: string; phone?: string } = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'forgot') {
        ...
        const otpSent = await sendOtp(phone, userId);
        if (otpSent) {
          setVerificationStep(true);
          startResendTimer();
        } else {
          // If OTP fails, still show email confirmation message
                ...
                <div className="text-center">
                  <p className="text-sm text-muted-foreground">
                    ...
                    <button
                      ...
                      className={`font-medium ${resendTimer > 0 ? 'text-muted-foreground cursor-not-allowed' : 'text-accent hover:underline'}`}
                    >
