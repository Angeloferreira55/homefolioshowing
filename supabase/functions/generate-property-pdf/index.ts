...
serve(async (req) => {
  ...
  try {
    ...
    // Embed document PDFs if possible
    if (documents && documents.length > 0) {
      for (const doc of documents) {
        try {
          ...
          if (contentType.includes("pdf")) {
            ...
            try {
              ...
              const sepPage = pdfDoc.addPage([pageWidth, pageHeight]);
              sepPage.drawText(doc.name, {
                x: margin,
                y: pageHeight / 2,
                size: 18,
                font: helveticaBold,
              ...
              sepPage.drawText(getDocTypeLabel(doc.doc_type), {
                x: margin,
                y: pageHeight / 2 - 25,
                size: 12,
                font: helvetica,
            ...
            try {
              ...
              if (image) {
                ...
                // Add document name header
                imgPage.drawText(doc.name, {
                  x: margin,
                  y: pageHeight - margin,
                  size: 14,
                  font: helveticaBold,
                ...
                let scale = Math.min(maxImgWidth / imgDims.width, maxImgHeight / imgDims.height, 1);
                
                imgPage.drawImage(image, {
                  x: margin,
                  y: pageHeight - margin - 30 - imgDims.height * scale,
