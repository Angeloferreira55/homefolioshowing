-- ============================================================
-- Migration: Create Managed Agents system for Assistant/TC tier
-- ============================================================

-- Table: managed_agents
-- Stores agent profiles managed by a Transaction Coordinator (TC).
-- Each TC can manage up to 5 agent profiles under their single login.
...
-- Public read access for public session display
...
-- When set, the session displays the managed agent's branding.
-- When NULL, existing behavior (session owner's profile) is used.
-- ============================================================
...
-- ============================================================
-- RPC: Resolve agent profile for public session display
-- Returns the managed agent's profile if agent_profile_id is set,
-- otherwise falls back to the session owner's own profile.
-- ============================================================
...
AS $$
  SELECT
    COALESCE(ma.full_name, p.full_name),
    COALESCE(ma.avatar_url, p.avatar_url),
    COALESCE(ma.slogan, p.slogan),
    COALESCE(ma.bio, p.bio),
    COALESCE(ma.phone, p.phone),
    COALESCE(ma.email, p.email),
    COALESCE(ma.company, p.company),
    COALESCE(ma.brokerage_name, p.brokerage_name),
    COALESCE(ma.brokerage_address, p.brokerage_address),
    COALESCE(ma.brokerage_phone, p.brokerage_phone),
    COALESCE(ma.brokerage_email, p.brokerage_email),
    COALESCE(ma.brokerage_logo_url, p.brokerage_logo_url),
    COALESCE(ma.linkedin_url, p.linkedin_url),
    COALESCE(ma.instagram_url, p.instagram_url),
    COALESCE(ma.facebook_url, p.facebook_url),
    COALESCE(ma.twitter_url, p.twitter_url),
    COALESCE(ma.youtube_url, p.youtube_url),
    COALESCE(ma.website_url, p.website_url)
...
-- Same function but by session ID (for PDF generation)
